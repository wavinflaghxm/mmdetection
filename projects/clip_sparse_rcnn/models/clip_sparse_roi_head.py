...
class CLIPSparseRoIHead(CascadeRoIHead):
    r"""
    ...
    def __init__(self,
        ...
        super(CLIPSparseRoIHead, self).__init__(
            num_stages,
            stage_loss_weights,
    ...
    def _bbox_forward(self, stage, x, rois, object_feats, img_metas, labels=None):
        ...
        cls_score, bbox_pred, object_feats, attn_feats = bbox_head(
            bbox_feats, object_feats, labels)
    ...
    def forward_train(self,
        ...
        for stage in range(self.num_stages):
            ...
            sampling_labels = self._label_sampler(gt_labels)
            bbox_results = self._bbox_forward(stage, x, rois, object_feats,
                                              img_metas, sampling_labels)
            ...
            labels, label_weights, bbox_targets, bbox_weights = self.bbox_head[stage].get_targets(
            ...
            if self.bbox_head[0].loss_cls.use_sigmoid:
                fed_weight = fed_weight[:num_classes]
            else:
                fed_weight[num_classes] = 1.0
            label_weights = label_weights.view(-1, 1) * fed_weight.view(1, -1)
    ...
    def simple_test(self,
        ...
        for img_id in range(num_imgs):
            ...
            if self.test_cfg.get('proposal', False):
                cls_score_per_img, labels_per_img = cls_score_per_img.max(dim=1)
                scores_per_img, topk_indices = cls_score_per_img.topk(
                ...
                if self.test_cfg.get('nms', None) is not None:
                    det_bbox, det_label = multiclass_nms(
                        proposal_list[img_id],
                        cls_score_per_img,
                        self.test_cfg.score_thr,
                        self.test_cfg.nms,
                        self.test_cfg.max_per_img)
                    bbox_pred_per_img = det_bbox[:, :4]
                    scores_per_img = det_bbox[:, 4]
                    labels_per_img = det_label
                else:
        ...
        if self.test_cfg.get('proposal', False):
            bbox_results = [bbox.detach().cpu().numpy() for bbox in det_bboxes]
        else:
            bbox_results = [
    ...
    def _label_sampler(self,
        ...
        appeared = torch.unique(gt_labels)
        prob = appeared.new_ones(num_classes + 1).float()
        prob[-1] = 0
        if len(appeared) < num_sample_classes:
            ...
            more_appeared = torch.multinomial(
                prob, num_sample_classes - len(appeared), replacement=False)
